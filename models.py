...
class GhostDatabase:
    ...
    def list_random_three(self):
        ghosts = ndb.GqlQuery("SELECT * FROM Ghost")
    ...
    def list_all(self):
        return ndb.GqlQuery("SELECT * FROM Ghost")

    def get_from_user(self, user):
        if user:
            matches = ndb.GqlQuery("SELECT * FROM Ghost WHERE gmail = :value", value=user)
            try:
                return matches[0]
            except IndexError:
    ...
    def get_from_ghostname(self, name):
        matches = ndb.GqlQuery("SELECT * FROM Ghost WHERE ghost_name = :value", value=name)
        return matches[0]
